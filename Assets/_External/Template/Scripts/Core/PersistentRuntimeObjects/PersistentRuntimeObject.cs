...
namespace Template.Core
{
    ...
    {
        protected static void CreateObjectInstance(string defaultObjectName)
        {
            DontDestroyOnLoad(new GameObject(defaultObjectName, typeof(TObject)));
        }
    ...
    public abstract class PersistentRuntimeObject<TObject, TData> : PersistentRuntimeObject<TObject>, IPersistentRuntimeObject<TData> where TObject : MonoBehaviour, IPersistentRuntimeObject<TData> where TData : PersistentRuntimeObjectData<TData>
    {
        ...
        protected virtual void Awake()
        {
            ...
            enabled = PersistentData.Settings.Enabled;
            gameObject.SetActive(PersistentData.Settings.Active);
            gameObject.isStatic = PersistentData.Settings.Static;

            if (PersistentData.Settings.ShouldOverrideDefaultName)
                gameObject.name = PersistentData.Settings.NameOverride;
        }
    }
